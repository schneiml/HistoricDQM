class ChartList {
    constructor(dataset, charts) {
        ...
        charts.forEach(function (c) { 
            self.map[c.name] = self.charts.length;
            if (c.corr && c.files.length == 2)
                self.charts.push(new CorrPlot(c, i++))
            else
                self.charts.push(new XRangePlot(c, i++));
        });
        this.need_update = true;
        this.page = 1;
        this.dataset = dataset;
        const per_page_el = $("#per-page");
        ...
        $(".runs-input").find("input").change(function() {
    ...
    init_list() {
        const self = this;
        var cnt = 1;
        const list_el = $("#list").html("");
        const list_page = '<li class="list-page""></li>';
        var list_page_el = $(list_page).appendTo(list_el);
        for (var i = 0; i < this.charts.length; i++) {
            const chart = this.charts[i];
            const chart_name = chart.name;
            const list_item = '<div><input type="checkbox"><a></a></div>';
            const item_el = $(list_item).appendTo(list_page_el);
            chart.list_item = item_el
            const checkbox_el = item_el.find("input").prop("checked", true);
